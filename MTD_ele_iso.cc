...
std::tuple<int,float,float,int,float,float> MTD_Ele_iso::ele_iso(const reco::GsfTrack* eleTrack) const {
    ...
    int N_tracks_MTD = -1; // so we can seperate electrons that are not matched to primary vertex
    double pT_sum_MTD = -1;
    ...
    if (ele_track_source < dZvtxCut_){
      ...
      if (ele_SigTrkIdx >= 0) { // if we found a match, we add MTD timing information for this matched track and check if track is in vertex fit track collection
        ...
        if(Vtx_chosen.trackWeight(ele_sigTrkRef) > 0.5){
          int general_index = 0;
          ...
          if(N_tracks == -1 && pT_sum == -1){
            ++N_tracks; // if we have 0 tracks in electron iso cone, we get 0 track result, not -1 tracks, same for Ch_iso sum
            ++pT_sum;
            ++rel_pT_sum;

            ++N_tracks_MTD; // if regular noMTD case has 0 tracks, than ofcourse MTD one will have 0 aswell
            ++pT_sum_MTD;
            ++rel_pT_sum_MTD;
          }
