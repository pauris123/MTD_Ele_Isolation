...
std::tuple<int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,int,int> MTD_Ele_iso::ele_iso(const reco::GsfTrack* eleTrack) const {
    ...
    nTracks_from_vtx = Vtx_chosen.nTracks();
    ...
    //int ele_SigTrkIdx = -1; // used for IDing the matched electron track from general track collection // uncomment for timing stuff
    //int eletrkIdx = -1;  // uncomment for timing stuff

    // Checking if electron comes from primary vertex

    float ele_track_source_dz = 0;
    float ele_track_source_dxy = 0;
    int tracks_in_cone_full = -1;
    //ele_track_source_dz = fabs(eleTrack->vertex().z()-Vtx_chosen.z()); // eleTrack->vertex()
    ele_track_source_dz = fabs(eleTrack->dz(Vtx_chosen.position()));
    ele_track_source_dxy = fabs(eleTrack->dxy(Vtx_chosen.position()));


    // checking only elecctrons who's tracks are matched with primary vtx < 0.1 cut
    if (ele_track_source_dz < dZvtxCut_ && ele_track_source_dxy < dxyVtxCut_){
      
      /*//here starts the comment out electron track matching for MTD timing case
      ...
      */// here ends the comment out electron track matching for MTD timing case
        //if(Vtx_chosen.trackWeight(ele_sigTrkRef) > 0.5){ // ele track inside vtx track coll cut
          ...
          for (const auto& trackGen : *Track_coll_) { // looping over all the tracks to get charged rel_iso for electrons
            ...
            // Here the noMTD case ends
             ...
             /*// here starts the comment out for MTD timing case if it is needed to put it on again !!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            } */// here ends the commented out section for MTD timing case !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          } // looping over tracks ///
        ...
        //} // ele track inside vtx track collection cut
      //} // electron track inside general track collection, for timing stuff
    } // dz and dxy 
    

std::tuple<int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,int,int> iso_output;
...
N_tracks_MTD_5,pT_sum_MTD_5,rel_pT_sum_MTD_5 , N_tracks_MTD_6,pT_sum_MTD_6,rel_pT_sum_MTD_6 , N_tracks_MTD_7,pT_sum_MTD_7,rel_pT_sum_MTD_7, tracks_in_cone_full, Vertex_Number, nTracks_from_vtx);
