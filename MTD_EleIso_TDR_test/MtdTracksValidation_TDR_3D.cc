...
void MtdTracksValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if(electron_iso_calc_){
    ...
    float min_pt_cut = 0.7;
    float max_dz_cut = 0.1; // will check 0.15/0.20/0.25/0.30
    ...
    //GenRecTrackHandle
    MTD_Ele_iso iso_object(min_dR_cut,
                          max_dR_cut,
                          min_pt_cut,
                          max_dz_cut,
                          max_dz_vtx_cut,
    ...
    for (const auto& ele : eleColl){
      ...
      if( ele.pt()> 10 && fabs(ele.eta()) < 2.4 ){
        ...
        for(const auto& genParticle : GenPartColl){
          ...
          if(genParticle.pdgId() == 11 || genParticle.pdgId() == -11){
            ...
            if(genParticle.mother()->pdgId() == 23){
              ...
              if(genParticle.mother()->numberOfDaughters() == 2){
        ...
        if(eleMatch_found){
          ...
          std::tuple<int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int> iso_return;
...
void MtdTracksValidation::bookHistograms(DQMStore::IBooker& ibook, edm::Run const& run, edm::EventSetup const& iSetup) {
  ...
  meEleISO_Ntracks_ = ibook.book1D("Ele_Iso_Ntracks", "Tracks in isolation cone around electron track after basic cuts", 10, 0, 10); // hists for electrons
  meEleISO_chIso_ = ibook.book1D("Ele_chIso_sum", "Track pT sum in isolation cone around electron track after basic cuts", 200, 0, 10);
  meEleISO_rel_chIso_ = ibook.book1D("Ele_rel_chIso_sum", "Track relative pT sum in isolation cone around electron track after basic cuts", 100, 0, 2);

  meEleISO_Ntracks_MTD_1_ = ibook.book1D("Ele_Iso_Ntracks_MTD_1", "Tracks in isolation cone around electron track after basic cuts with MTD", 10, 0, 10); // hists for electrons
  meEleISO_chIso_MTD_1_ = ibook.book1D("Ele_chIso_sum_MTD_1", "Track pT sum in isolation cone around electron track after basic cuts with MTD", 200, 0, 10);
  meEleISO_rel_chIso_MTD_1_ = ibook.book1D("Ele_rel_chIso_sum_MTD_1", "Track relative pT sum in isolation cone around electron track after basic cuts with MTD", 100, 0, 2);

  meEleISO_Ntracks_MTD_2_ = ibook.book1D("Ele_Iso_Ntracks_MTD_2", "Tracks in isolation cone around electron track after basic cuts with MTD", 10, 0, 10); // hists for electrons
  meEleISO_chIso_MTD_2_ = ibook.book1D("Ele_chIso_sum_MTD_2", "Track pT sum in isolation cone around electron track after basic cuts with MTD", 200, 0, 10);
  meEleISO_rel_chIso_MTD_2_ = ibook.book1D("Ele_rel_chIso_sum_MTD_2", "Track relative pT sum in isolation cone around electron track after basic cuts with MTD", 100, 0, 2);

  meEleISO_Ntracks_MTD_3_ = ibook.book1D("Ele_Iso_Ntracks_MTD_3", "Tracks in isolation cone around electron track after basic cuts with MTD", 10, 0, 10); // hists for electrons
  meEleISO_chIso_MTD_3_ = ibook.book1D("Ele_chIso_sum_MTD_3", "Track pT sum in isolation cone around electron track after basic cuts with MTD", 200, 0, 10);
  meEleISO_rel_chIso_MTD_3_ = ibook.book1D("Ele_rel_chIso_sum_MTD_3", "Track relative pT sum in isolation cone around electron track after basic cuts with MTD", 100, 0, 2);

  meEleISO_Ntracks_MTD_4_ = ibook.book1D("Ele_Iso_Ntracks_MTD_4", "Tracks in isolation cone around electron track after basic cuts with MTD", 10, 0, 10); // hists for electrons
  meEleISO_chIso_MTD_4_ = ibook.book1D("Ele_chIso_sum_MTD_4", "Track pT sum in isolation cone around electron track after basic cuts with MTD", 200, 0, 10);
  meEleISO_rel_chIso_MTD_4_ = ibook.book1D("Ele_rel_chIso_sum_MTD_4", "Track relative pT sum in isolation cone around electron track after basic cuts with MTD", 100, 0, 2);

  meEleISO_Ntracks_MTD_5_ = ibook.book1D("Ele_Iso_Ntracks_MTD_5", "Tracks in isolation cone around electron track after basic cuts with MTD", 10, 0, 10); // hists for electrons
  meEleISO_chIso_MTD_5_ = ibook.book1D("Ele_chIso_sum_MTD_5", "Track pT sum in isolation cone around electron track after basic cuts with MTD", 200, 0, 10);
  meEleISO_rel_chIso_MTD_5_ = ibook.book1D("Ele_rel_chIso_sum_MTD_5", "Track relative pT sum in isolation cone around electron track after basic cuts with MTD", 100, 0, 2);

  meEleISO_Ntracks_MTD_6_ = ibook.book1D("Ele_Iso_Ntracks_MTD_6", "Tracks in isolation cone around electron track after basic cuts with MTD", 10, 0, 10); // hists for electrons
  meEleISO_chIso_MTD_6_ = ibook.book1D("Ele_chIso_sum_MTD_6", "Track pT sum in isolation cone around electron track after basic cuts with MTD", 200, 0, 10);
  meEleISO_rel_chIso_MTD_6_ = ibook.book1D("Ele_rel_chIso_sum_MTD_6", "Track relative pT sum in isolation cone around electron track after basic cuts with MTD", 100, 0, 2);

  meEleISO_Ntracks_MTD_7_ = ibook.book1D("Ele_Iso_Ntracks_MTD_7", "Tracks in isolation cone around electron track after basic cuts with MTD", 10, 0, 10); // hists for electrons
  meEleISO_chIso_MTD_7_ = ibook.book1D("Ele_chIso_sum_MTD_7", "Track pT sum in isolation cone around electron track after basic cuts with MTD", 200, 0, 10);
  meEleISO_rel_chIso_MTD_7_ = ibook.book1D("Ele_rel_chIso_sum_MTD_7", "Track relative pT sum in isolation cone around electron track after basic cuts with MTD", 100, 0, 2);


  meEle_pt_tot_ = ibook.book1D("Ele_pT_tot", "Electron pT tot", 50, 0, 100); // hists for ele isto stuff start
  meEle_pt_noMTD_ = ibook.book1D("Ele_pT_noMTD", "Electron pT noMTD", 50, 0, 100);

  meEle_eta_tot_ = ibook.book1D("Ele_eta_tot", "Electron eta tot", 64, -3.2, 3.2);
  meEle_eta_noMTD_ = ibook.book1D("Ele_eta_noMTD", "Electron eta noMTD", 64, -3.2, 3.2);

  meEle_phi_tot_ = ibook.book1D("Ele_phi_tot", "Electron phi tot", 64, -3.2, 3.2);
  meEle_phi_noMTD_ = ibook.book1D("Ele_phi_noMTD", "Electron phi noMTD", 64, -3.2, 3.2);

  
  meEle_pt_MTD_1_ = ibook.book1D("Ele_pT_MTD_1", "Electron pT MTD", 50, 0, 100);
  meEle_eta_MTD_1_ = ibook.book1D("Ele_eta_MTD_1", "Electron eta MTD", 64, -3.2, 3.2);
  meEle_phi_MTD_1_ = ibook.book1D("Ele_phi_MTD_1", "Electron phi MTD", 64, -3.2, 3.2);

  meEle_pt_MTD_2_ = ibook.book1D("Ele_pT_MTD_2", "Electron pT MTD", 50, 0, 100);
  meEle_eta_MTD_2_ = ibook.book1D("Ele_eta_MTD_2", "Electron eta MTD", 64, -3.2, 3.2);
  meEle_phi_MTD_2_ = ibook.book1D("Ele_phi_MTD_2", "Electron phi MTD", 64, -3.2, 3.2);

  meEle_pt_MTD_3_ = ibook.book1D("Ele_pT_MTD_3", "Electron pT MTD", 50, 0, 100);
  meEle_eta_MTD_3_ = ibook.book1D("Ele_eta_MTD_3", "Electron eta MTD", 64, -3.2, 3.2);
  meEle_phi_MTD_3_ = ibook.book1D("Ele_phi_MTD_3", "Electron phi MTD", 64, -3.2, 3.2);

  meEle_pt_MTD_4_ = ibook.book1D("Ele_pT_MTD_4", "Electron pT MTD", 50, 0, 100);
  meEle_eta_MTD_4_ = ibook.book1D("Ele_eta_MTD_4", "Electron eta MTD", 64, -3.2, 3.2);
  meEle_phi_MTD_4_ = ibook.book1D("Ele_phi_MTD_4", "Electron phi MTD", 64, -3.2, 3.2);

  meEle_pt_MTD_5_ = ibook.book1D("Ele_pT_MTD_5", "Electron pT MTD", 50, 0, 100);
  meEle_eta_MTD_5_ = ibook.book1D("Ele_eta_MTD_5", "Electron eta MTD", 64, -3.2, 3.2);
  meEle_phi_MTD_5_ = ibook.book1D("Ele_phi_MTD_5", "Electron phi MTD", 64, -3.2, 3.2);

  meEle_pt_MTD_6_ = ibook.book1D("Ele_pT_MTD_6", "Electron pT MTD", 50, 0, 100);
  meEle_eta_MTD_6_ = ibook.book1D("Ele_eta_MTD_6", "Electron eta MTD", 64, -3.2, 3.2);
  meEle_phi_MTD_6_ = ibook.book1D("Ele_phi_MTD_6", "Electron phi MTD", 64, -3.2, 3.2);

  meEle_pt_MTD_7_ = ibook.book1D("Ele_pT_MTD_7", "Electron pT MTD", 50, 0, 100);
  meEle_eta_MTD_7_ = ibook.book1D("Ele_eta_MTD_7", "Electron eta MTD", 64, -3.2, 3.2);
  meEle_phi_MTD_7_ = ibook.book1D("Ele_phi_MTD_7", "Electron phi MTD", 64, -3.2, 3.2);
  ...
  meEle_test_ConeTracks_ = ibook.book1D("Ele_ConeTracks_test","The amount of tracks inside electron isolation cone with any dz/dxy/dt/pT cuts",100,0,100);
  meVtx_test_ = ibook.book1D("Vtx_test", "Test values", 20, 0, 2);
...
void MtdTracksValidation::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("inputTagV", edm::InputTag("offlinePrimaryVertices")); // might want to try "offlinePrimaryVertices" / "offlinePrimaryVertices4D" -> 4D was the original one used
  desc.add<edm::InputTag>("inputTagH", edm::InputTag("generatorSmeared"));

  //desc.add<edm::InputTag>("inputEle", edm::InputTag("gedGsfElectrons")); // Adding the electron collection name ! (At least I think I'm doing that)
  desc.add<edm::InputTag>("inputEle", edm::InputTag("ecalDrivenGsfElectrons"));
  ...
  desc.addUntracked<bool>("optionalEleIso", true); // Added option to calculate ele iso
