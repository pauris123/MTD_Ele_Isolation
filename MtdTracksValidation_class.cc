...
void MtdTracksValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if(electron_iso_calc_){
    ...
    float max_dz_cut = 0.3; // will check 0.15/0.20/0.25/0.30
    float max_dz_vtx_cut = 0.1; 
    ...
    /*if (primRecoVtx.nTracks() > 20){
      meVtx_test_->Fill(0);
    }

    for(auto const &vtx_track : primRecoVtx.tracks()){

      meVtx_test_->Fill(vtx_track->pt());
    ...
    for (const auto& ele : eleColl){
      ...
      std::tuple<int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float,int,float,float> iso_return;
...
void MtdTracksValidation::bookHistograms(DQMStore::IBooker& ibook, edm::Run const& run, edm::EventSetup const& iSetup) {
  ...
  meEleISO_chIso_ = ibook.book1D("Ele_chIso_sum", "Track pT sum in isolation cone around electron track after basic cuts", 100, 0, 5);
  ...
  meEle_pt_tot_ = ibook.book1D("Ele_pT_tot", "Electron pT tot", 100, 0, 100); // hists for ele isto stuff start
  ...
  meEle_pt_MTD_1_ = ibook.book1D("Ele_pT_MTD_1", "Electron pT MTD", 100, 0, 100);
  meEle_eta_MTD_1_ = ibook.book1D("Ele_eta_MTD_1", "Electron eta MTD", 100, -3.2, 3.2);
  meEle_phi_MTD_1_ = ibook.book1D("Ele_phi_MTD_1", "Electron phi MTD", 100, -3.2, 3.2);

  meEle_pt_MTD_2_ = ibook.book1D("Ele_pT_MTD_2", "Electron pT MTD", 100, 0, 100);
  meEle_eta_MTD_2_ = ibook.book1D("Ele_eta_MTD_2", "Electron eta MTD", 100, -3.2, 3.2);
  meEle_phi_MTD_2_ = ibook.book1D("Ele_phi_MTD_2", "Electron phi MTD", 100, -3.2, 3.2);

  meEle_pt_MTD_3_ = ibook.book1D("Ele_pT_MTD_3", "Electron pT MTD", 100, 0, 100);
  meEle_eta_MTD_3_ = ibook.book1D("Ele_eta_MTD_3", "Electron eta MTD", 100, -3.2, 3.2);
  meEle_phi_MTD_3_ = ibook.book1D("Ele_phi_MTD_3", "Electron phi MTD", 100, -3.2, 3.2);

  meEle_pt_MTD_4_ = ibook.book1D("Ele_pT_MTD_4", "Electron pT MTD", 100, 0, 100);
  meEle_eta_MTD_4_ = ibook.book1D("Ele_eta_MTD_4", "Electron eta MTD", 100, -3.2, 3.2);
  meEle_phi_MTD_4_ = ibook.book1D("Ele_phi_MTD_4", "Electron phi MTD", 100, -3.2, 3.2);

  meEle_pt_MTD_5_ = ibook.book1D("Ele_pT_MTD_5", "Electron pT MTD", 100, 0, 100);
  meEle_eta_MTD_5_ = ibook.book1D("Ele_eta_MTD_5", "Electron eta MTD", 100, -3.2, 3.2);
  meEle_phi_MTD_5_ = ibook.book1D("Ele_phi_MTD_5", "Electron phi MTD", 100, -3.2, 3.2);

  meEle_pt_MTD_6_ = ibook.book1D("Ele_pT_MTD_6", "Electron pT MTD", 100, 0, 100);
  meEle_eta_MTD_6_ = ibook.book1D("Ele_eta_MTD_6", "Electron eta MTD", 100, -3.2, 3.2);
  meEle_phi_MTD_6_ = ibook.book1D("Ele_phi_MTD_6", "Electron phi MTD", 100, -3.2, 3.2);

  meEle_pt_MTD_7_ = ibook.book1D("Ele_pT_MTD_7", "Electron pT MTD", 100, 0, 100);
  meEle_eta_MTD_7_ = ibook.book1D("Ele_eta_MTD_7", "Electron eta MTD", 100, -3.2, 3.2);
  meEle_phi_MTD_7_ = ibook.book1D("Ele_phi_MTD_7", "Electron phi MTD", 100, -3.2, 3.2);
...
void MtdTracksValidation::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("inputTagV", edm::InputTag("offlinePrimaryVertices4D")); // might want to try "offlinePrimaryVertices" / "offlinePrimaryVertices4D" -> 4D was the original one used
