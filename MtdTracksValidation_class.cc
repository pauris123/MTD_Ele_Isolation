...
void MtdTracksValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  const bool isGoodVtx = std::abs(primRecoVtx.z() - zsim) < deltaZcut_ || primRecoVtx.isFake(); // we are checking only 1 vertex and taking it as the primary one??????
  ...
  if(electron_iso_calc_){
    ...
    std::vector <reco::Vertex> vertices = *RecVertexHandle; 
     ...
     // This stuff still gives me Vtx_chosen "def out of scope"
