...
void MtdTracksValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if(electron_iso_calc_){
    ...
    for (const auto& ele : eleColl){
      ...
      const reco::GsfTrack* eleTrack_test = &(*ele.gsfTrack());
      ...
      math::XYZVector EleSigTrackMomentumAtVtx_test = eleTrack_test->momentum(); // not sure if correct, but works
      //double EleSigTrackEtaAtVtx_test = eleTrack_test->eta();


      int ele_SigTrkIdx_test = -1; // used for IDing the matched electron track from general track collection
      int eletrkIdx_test = -1; 

      
      for (const auto& trackGen_test : *GenRecTrackHandle) { // looping over tracks to find the match to electron track
          
        eletrkIdx_test++;
        //double dr = ROOT::Math::VectorUtil::DeltaR(trackGen->momentum(), EleSigTrackMomentumAtVtx);
        double dr_test = reco::deltaR(trackGen_test.momentum(), EleSigTrackMomentumAtVtx_test);

        int check_test = 0; 

        if (dr_test < min_dR_cut) 
        {
          ele_SigTrkIdx_test = eletrkIdx_test;
          check_test++; // should do a check if we match 2 or more tracks to one electron track
        }
       }

      reco::TrackRef ele_sigTrkRef_test; // matched electron track ref variable


      if (ele_SigTrkIdx_test >= 0) { // if we found a match, we add MTD timing information for this matched track
        
        const reco::TrackRef ele_sigTrkRef_test(iEvent.getHandle(GenRecTrackToken_), ele_SigTrkIdx_test);

        meVtx_test_->Fill(primRecoVtx.trackWeight(ele_sigTrkRef_test));
      }
      ...
      */
      ...
      /*
      meEle_track_pt_->Fill(ele.gsfTrack()->pt());
      meEle_track_eta_->Fill(ele.gsfTrack()->eta());
      meEle_track_phi_->Fill(ele.gsfTrack()->phi());
      meEle_track_vz_->Fill(ele.gsfTrack()->vz());*/
      ...
      //meEleISO_Ntracks_->Fill(iso_return.first); // Filling hists for Ntraks and chIso sums //
