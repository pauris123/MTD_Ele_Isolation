...
void MtdTracksValidation::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  // Here I will try to put in electron isolation code ///////////////////////////////////////////////////////
  //
  //
  //
  ...
  if(electron_iso_calc_){
    ...
    //std::vector<reco::GsfElectron> *eleColl = (eleHandle.product()); // diffrenet eleColl type I saw in compiler errors

    for (const auto& ele : eleColl) {

      meEle_pt_->Fill(ele.pt());
      meEle_phi_->Fill(ele.phi());
      meEle_eta_->Fill(ele.eta());
      

      meEle_test_->Fill(1.0); // testing eletron count
    
      // later on should divide barrel and endcap sections seperate
      ...
      meEle_track_eta_->Fill(EleSigTrackEtaAtVtx);
      
      //double Eleeta = ele.eta(); // testing stuff for compiler errors
      //double Elept = ele.pt();
      ...
      if (ele_SigTrkIdx >= 0) { // if we found a match, we add MTD timing information for this matched track
        ...
        //ele_sigTrkRef = reco::TrackRef(GenRecTrackHandle, ele_SigTrkIdx);
        ...
        ele_sigTrkTime = t0Pid[ele_sigTrkRef]; // matched electron track mtd time
        ele_sigTrkTimeErr = Sigmat0Pid[ele_sigTrkRef]; // matched electron track mtd time error
        ele_sigTrkMtdMva = mtdQualMVA[ele_sigTrkRef]; // matched electron track mtd MVA score
    
        ele_sigTrkTimeErr = (ele_sigTrkMtdMva > min_track_mtd_mva_cut)? ele_sigTrkTimeErr: -1; // track MTD MVA cut/check
      }

      int general_index = 0;
      for (const auto& trackGen : *GenRecTrackHandle) { // looping over all the tracks to get charged rel_iso for electrons
        ...
        if (trackGen.pt() < min_pt_cut ){ // track pT cut, might ignore at the first iterations of the code
          continue;
        }

        if (fabs(trackGen.vz() - ele.gsfTrack()->vz()) > max_dz_cut ){ // should do some dz or some other Z axis cut/check
          continue;
        }
        ...
        double TrkMTDTime = t0Pid[trackref_general]; // MTD timing info for particular track
        ...
        if(isGoodVtx && TrkMTDTimeErr > 0 && primRecoVtx.tError() > 0){
          ...
          if(dt_vtx > max_dt_cut){ // if dt larger then dtCut, then we skip this track
            continue;
          }
        ...
        if(TrkMTDTimeErr > 0 && ele_sigTrkTimeErr > 0){
          ...
          if(dt_sigTrk > max_dt_cut){ // if dt larger then dtCut, then we skip this track
            continue;
          }
        ...
        if(dr_check > min_dR_cut && dr_check < max_dR_cut && deta > min_strip_cut){
          ++N_tracks;
          pT_sum += trackGen.pt(); 
        }
      ...
      rel_pT_sum = pT_sum/ele.pt();

      meEleISO_Ntracks_->Fill(N_tracks); // Filling hists for Ntraks and chIso sums
      meEleISO_chIso_->Fill(pT_sum);
      meEleISO_rel_chIso_->Fill(rel_pT_sum);

    } // looping over electrons
  } // bool iso statement check
  ...
  //
  //
  // Here we finish electron isolation stuffies /////////////////////////////////////////////////////////////////////////
